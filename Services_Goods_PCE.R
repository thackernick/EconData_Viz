...
#### Pulling GDP from Fred #### 


goods = fredr('DGDSRG3M086SBEA',
            observation_start = start_date, 
            observation_end = end_date,
            units='pc1')
goods = goods[,c(1,3)]
goods = goods %>% rename('Date' = date, 
                     'Goods' = value)




data = goods %>% filter(Date>= start_date)


services = fredr('DSERRG3M086SBEA',
              observation_start = start_date, 
              observation_end = end_date,
              units = 'pc1')
services = services[,c(1,3)]
services = services %>% rename('Date' = date, 
                         'Services' = value)




dataserv = services %>% filter(Date>= start_date)


data = full_join(data, dataserv, by='Date')
...
p1 = {
  ggplot(data) + 
    ...
    geom_line(aes(x = Date, y = Goods, color = 'PCE Goods Inflation'), size = 1.5) + #GDP line
    geom_line(aes(x = Date, y = Services, color = 'PCE Services Inflation'), size = 1.5) +
    #ggtitle('Goods and Services Inf Graph') + 
    ylab('Percent Change From Year Ago') + 
    xlab('Source: Bureau of Economic Analysis  | Shaded Area Indicates US Recession') + 
    scale_y_continuous(breaks = seq(-2,10,2), limits=c(-3,NA)) + #Bounds of plot
    scale_x_date(date_labels = '%Y') +  
    coord_cartesian(expand = FALSE) + #turn off axis expansion (padding)
    theme_ipsum() + 
    scale_color_manual(name = '', values = c('PCE Goods Inflation' = paint[1],'PCE Services Inflation' = paint[2])) +
    theme(
...
ggsave(dpi = "retina", plot = p1, "inflation_goods_and_services.png", type = "cairo-png",
