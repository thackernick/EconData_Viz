...
####Data pull####
taylor <- read_excel("taylor-rule-data.xlsx", sheet = 'Sheet1')
taylor$date <- as.Date(taylor$date)
...
#### Plot #### 
p1 <- ggplot(taylor_filtered) + 
  # Add shaded recession bars
  geom_rect(data = recessions_filtered, aes(xmin = Peak, xmax = Trough, ymin = -Inf, ymax = +Inf), 
            fill = 'grey', alpha = 0.5) +
  # Plot Taylor rule estimate
  geom_line(aes(x = date, y = taylor, color = 'Taylor Rule Estimate'), size = 1.5) +
  # Plot actual federal funds rate (FFR)
  geom_line(aes(x = date, y = ffr, color = 'Federal Funds Rate'), size = 1.5) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", size = 0.8) +
  # Customize labels
  ylab('Percent') +
  xlab('Source: Federal Reserve Bank of Atlanta | Shaded Area Indicates US Recession') +
  
  scale_y_continuous(breaks = seq(-20, 10, 5), limits = c(-20, NA)) +
  scale_x_date(date_labels = '%Y', limits = c(start_date, end_date)) +
  # Set color scheme
  scale_color_manual(name = '', values = c('Taylor Rule Estimate' = paint[1], 'Federal Funds Rate' = paint[2])) +
...
ggsave(dpi = "retina", plot = p1, "taylor_rule.png", type = "cairo-png",
